...
class CoopOptim(nn.Module):
    ...
    def calc_rh_loss(self, batch, net_output, stage, obj_faces, itr):
        ...
        o_mesh = Meshes(verts = self.obj_verts, faces = torch.from_numpy(obj_faces[None,:].astype(np.int64)).cuda())
    ...
    def fitting(self, batch, net_output_hands, net_output_body, obj_faces, r):
        ...
        for stg, optimizer in enumerate(self.optimizers):
            for itr in range(self.num_iters[stg]):
                optimizer.zero_grad()
                losses_rh, _, _ = self.calc_rh_loss(batch, net_output_hands, stg, obj_faces, itr)
